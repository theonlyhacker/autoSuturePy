...
class UNext(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=1, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.res_enconder1 = nn.Conv3d(1, 16, 1, 1)
        self.encoder2 = nn.Conv3d(16, 32, 3, stride=1, padding=1)
        self.res_enconder2 = nn.Conv3d(16, 32, 1, 1)
        self.encoder3 = nn.Conv3d(32, 128, 3, stride=1, padding=1)
        self.res_enconder3 = nn.Conv3d(32, 128, 1, 1)
        ...
        self.res_decoder1 = nn.Conv3d(128,32,1,1)
        self.decoder4 = nn.Conv3d(32, 16, 3, stride=1, padding=1)
        self.res_decoder2 = nn.Conv3d(32, 16, 1, 1)
        self.decoder5 = nn.Conv3d(16, 16, 3, stride=1, padding=1)
        self.res_decoder3 = nn.Conv3d(16, 16, 1, 1)
    ...
    def forward(self, x):
        ...
        out = F.relu(self.ebn1(self.encoder1(x))) + self.res_enconder1(x)
        out = F.relu(out)
        out = F.max_pool3d(out,2,2)
        t1 = out
        ### Stage 2
        # out = F.relu(F.max_pool3d(self.ebn2(self.encoder2(out)), 2, 2))  # (2,32,22,36,56)
        out = F.relu(self.ebn2(self.encoder2(out))) + self.res_enconder2(out)
        out = F.relu(out)
        out = F.max_pool3d(out, 2, 2)
        t2 = out
        ### Stage 3
        # out = F.relu(F.max_pool3d(self.ebn3(self.encoder3(out)), 2, 2))  # (2,128,11,18,28)
        out = F.relu(self.ebn3(self.encoder3(out))) + self.res_enconder3(out)
        out = F.relu(out)
        out = F.max_pool3d(out, 2, 2)
        ...
        out = F.relu(self.dbn3(self.decoder3(out))) + self.res_decoder1(out)
        out = F.relu(F.interpolate(out, scale_factor=(2, 2, 2), mode='trilinear'))  # (2,32,22,36,56)
        ...
        out = F.relu(self.dbn4(self.decoder4(out))) + self.res_decoder2(out)
        out = F.relu(F.interpolate(out, scale_factor=(2, 2, 2), mode='trilinear'))
        ...
        out = F.relu((self.decoder5(out))) + self.res_decoder3(out)
        out = F.relu(F.interpolate(out, scale_factor=(2, 2, 2), mode='trilinear'))
