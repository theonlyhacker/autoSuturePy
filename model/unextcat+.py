...
class UNext(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=1, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.dblock1 = nn.ModuleList([shiftedBlock(
            dim=320, num_heads=num_heads[0], mlp_ratio=1, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[0], norm_layer=norm_layer,
            sr_ratio=sr_ratios[0])])

        self.dblock2 = nn.ModuleList([shiftedBlock(
            dim=256, num_heads=num_heads[0], mlp_ratio=1, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[1], norm_layer=norm_layer,
            sr_ratio=sr_ratios[0])])
        ...
        self.decoder2 = nn.Conv3d(320, 128, 3, stride=1, padding=1)
        self.decoder3 = nn.Conv3d(256, 32, 3, stride=1, padding=1)
        self.decoder4 = nn.Conv3d(64, 16, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        ### Stage 1
        out = F.relu(self.ebn1(self.encoder1(x)))  # (2,16,44,72,112)
        t1 = out
        out = F.max_pool3d(out,2,2)

        ### Stage 2
        out = F.relu(self.ebn2(self.encoder2(out)))  # (2,32,22,36,56)
        t2 = out
        out = F.max_pool3d(out, 2, 2)

        ### Stage 3
        out = F.relu(self.ebn3(self.encoder3(out)))  # (2,128,11,18,28)
        t3 = out
        out = F.max_pool3d(out, 2, 2)
        ...
        out = torch.cat((out, t4), dim=1) # 桥这块用的是直接相加 shape(2,320,6,9,14)
        _, _, D, H, W = out.shape
        out = out.flatten(2).transpose(1, 2)  # shape(2,756,320)
        for i, blk in enumerate(self.dblock1):
            out = blk(out, D, H, W) # (2,756,320)  (D,H,W)=(6,9,14)

        ### Stage 3
        out = self.dnorm3(out)  # (2,756,320)
        ...
        out = F.pad(out,
                    [torch.div(diffZ, 2, rounding_mode="floor"), diffZ - torch.div(diffZ, 2, rounding_mode="floor"),
                     torch.div(diffX, 2, rounding_mode="floor"), diffX - torch.div(diffX, 2, rounding_mode="floor"),
                     torch.div(diffY, 2, rounding_mode="floor"), diffY - torch.div(diffY, 2, rounding_mode="floor")])
        out = torch.cat((out, t3),dim=1)  # (2,256,11,18,28) (D,H,W)=(11,18,28)
        _, _, D, H, W = out.shape
        out = out.flatten(2).transpose(1, 2)  # (2,256,5544)->(2,5544,256)
        for i, blk in enumerate(self.dblock2):
            out = blk(out, D, H, W)
        out = self.dnorm4(out)
        out = out.reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()  # (2,5544,256)->(2,256,11,18,28)

        out = F.interpolate(out,scale_factor=(2, 2, 2), mode='trilinear')
        out = F.relu(
        ...
        out = torch.cat((out, t2),dim=1)  # (2,64,22,36,56)
        ...
        out = torch.cat((out, t1),dim=1)  # (2,16,44,72,112)
        out = F.relu(self.dbn5(self.decoder5(out)))
